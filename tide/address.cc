...
namespace tide
{
    ...
    bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol)
    {
        ...
        if (error)
        {
            TIDE_LOG_DEBUG(g_logger) << "Address::Lookup getaddrinfo(" << host << ", " << service << ") err=" << error << " errstr=" << gai_strerror(error);
            return false;
        }
    ...
    bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family)
    {
        ...
        {
            TIDE_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs error";
            return false;
        }
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        {
            TIDE_LOG_INFO(g_logger) << "inet_pton failed for " << address << ":" << port;
            return nullptr;
        }
    ...
    std::ostream &IPv4Address::insert(std::ostream &os) const
    {
        ...
        uint16_t port = byteswapOnLittleEndian(m_addr.sin_port);
        os << ((addr >> 24) & 0xFF) << "."
           << ((addr >> 16) & 0xFF) << "."
           << ((addr >> 8) & 0xFF) << "."
           << (addr & 0xFF) << ":" << port;
    ...
    IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
    {
        ...
        addr.sin6_port = byteswapOnLittleEndian(port);
    ...
    std::ostream &IPv6Address::insert(std::ostream &os) const
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            os << std::hex << byteswapOnLittleEndian(addr[i]);
        }
