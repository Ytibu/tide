...
static tide::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_worker_config
    = tide::Config::Lookup("workers", std::map<std::string, std::map<std::string, std::string> >(), "woker config");

WorkerGroup::WorkerGroup(uint32_t batch_size, tide::Scheduler* s)
...
bool WorkerManager::init() {
    ...
    for(auto& i : workers) {
        ...
        int32_t thread_num = tide::GetParamValue(i.second, "thread_num", 1);
        int32_t worker_num = tide::GetParamValue(i.second, "worker_num", 1);
