...
namespace tide
{
    ...
    static tide::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_sqlite3_dbs = tide::Config::Lookup("sqlite3.dbs", std::map<std::string, std::map<std::string, std::string>>(), "sqlite3 dbs");
    ...
    {
        // 创建事务对象并立即尝试开始事务
        // 如果 `begin()` 成功，则返回事务智能指针；失败则返回 nullptr
    ...
    ISQLData::ptr SQLite3::query(const char *format, ...)
    {
        ...
        // `SQLite3Stmt::query()` 返回一个 `SQLite3Data` 封装，用户通过
        // 调用 `next()` 迭代行数据并通过 `get*()` 访问列
        return stmt->query();
    }
    ...
    {
        return bind(idx, tide::Time2Str(value));
    }
    ...
    int SQLite3Stmt::prepare(const char *stmt)
    {
        ...
        // 使用 `sqlite3_prepare_v2` 编译 SQL 文本为 sqlite3_stmt
        // 注意：m_stmt 在 finish() 中会被 finalize，确保先释放旧的 stmt
    ...
    time_t SQLite3Data::getTime(int idx)
    {
        ...
        return tide::Str2Time(str.c_str());
    }
    ...
    SQLite3Transaction::~SQLite3Transaction()
    {
        if (m_status == 1)
        {
            ...
            if (m_status == 1)
            {
                TIDE_LOG_ERROR(g_logger) << m_db << " auto_commit=" << m_autoCommit << " fail";
            }
        }
    }
    ...
    SQLite3::ptr SQLite3Manager::get(const std::string &name)
    {
        ...
        lock.unlock();
        std::string path = tide::GetParamValue<std::string>(args, "path");
        if (path.empty())
        {
            TIDE_LOG_ERROR(g_logger) << "open db name=" << name << " path is null";
            return nullptr;
        }

        if (path.find(":") == std::string::npos)
        {
            path = tide::EnvMgr::GetInstance()->getAbsolutePath(path);
        }

        sqlite3 *db;
        if (sqlite3_open_v2(path.c_str(), &db, SQLite3::CREATE | SQLite3::READWRITE, nullptr))
        {
            TIDE_LOG_ERROR(g_logger) << "open db name=" << name << " path=" << path << " fail";
            return nullptr;
        }

        SQLite3 *rt = new SQLite3(db);
        std::string sql = tide::GetParamValue<std::string>(args, "sql");
        if (!sql.empty())
        {
            if (rt->execute(sql))
            {
                TIDE_LOG_ERROR(g_logger) << "execute sql=" << sql
    ...
    int SQLite3Manager::execute(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            TIDE_LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name
                                     << ") fail, format=" << format;
            return -1;
        }
    ...
    ISQLData::ptr SQLite3Manager::query(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            TIDE_LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name
                                     << ") fail, format=" << format;
            return nullptr;
        }
    ...
    SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string &name, bool auto_commit)
    {
        ...
        if (!conn)
        {
            TIDE_LOG_ERROR(g_logger) << "SQLite3Manager::openTransaction, get(" << name
                                     << ") fail";
            return nullptr;
        }
