...
namespace tide
{
    ...
    static tide::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_redis =
        tide::Config::Lookup("redis.config", std::map<std::string, std::map<std::string, std::string>>(), "redis config");
    ...
    Redis::Redis(const std::map<std::string, std::string> &conf)
    {
        ...
        m_port = tide::TypeUtil::Atoi(tmp.substr(pos + 1));
        m_passwd = get_value(conf, "passwd");
        m_logEnable = tide::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));
        ...
        uint64_t v = tide::TypeUtil::Atoi(tmp);
    ...
    bool Redis::connect(const std::string &ip, int port, uint64_t ms)
    {
        ...
        if (c)
        {
            ...
            if (!m_passwd.empty())
            {
                ...
                if (!r)
                {
                    TIDE_LOG_ERROR(g_logger) << "auth error:("
                                             << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (r->type != REDIS_REPLY_STATUS)
                {
                    TIDE_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                                             << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (!r->str)
                {
                    TIDE_LOG_ERROR(g_logger) << "auth reply str error: NULL("
                                             << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                ...
                {
                    TIDE_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                                             << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
            }
    ...
    ReplyPtr Redis::cmd(const char *fmt, va_list ap)
    {
        ...
        if (!r)
        {
            if (m_logEnable)
            {
                TIDE_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port << ")(" << m_name << ")";
            }
            return nullptr;
        }
    ...
    ReplyPtr Redis::cmd(const std::vector<std::string> &argv)
    {
        ...
        if (!r)
        {
            if (m_logEnable)
            {
                TIDE_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
            }
            return nullptr;
        }
        ReplyPtr rt(r, freeReplyObject);
    ...
    ReplyPtr Redis::getReply()
    {
        ...
        if (m_logEnable)
        {
            TIDE_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ...
    FoxRedis::FoxRedis(tide::FoxThread *thr, const std::map<std::string, std::string> &conf)
    ...
    void FoxRedis::OnAuthCb(redisAsyncContext *c, void *rp, void *priv)
    {
        ...
        if (strcmp(r->str, "OK") == 0)
        {
            TIDE_LOG_INFO(g_logger) << "auth ok: " << r->str << "("
                                    << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
        }
        else
        {
    ...
    void FoxRedis::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            TIDE_LOG_INFO(g_logger) << "FoxRedis::ConnectCb "
            ...
            if (!ar->m_passwd.empty())
            {
                ...
                if (rt)
                {
                    TIDE_LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
                }
            }
        }
        else
        {
            TIDE_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb "
                                     << c->c.tcp.host << ":" << c->c.tcp.port
                                     << " fail, error:" << c->errstr;
    ...
    {
        TIDE_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb "
                                << c->c.tcp.host << ":" << c->c.tcp.port
                                << " status:" << status;
        FoxRedis *ar = static_cast<FoxRedis *>(c->data);
        ar->m_status = UNCONNECTED;
    ...
    void FoxRedis::CmdCb(redisAsyncContext *ac, void *r, void *privdata)
    {
        ...
        if (ctx->timeout)
        {
            delete ctx;
            // if(ctx && ctx->fiber) {
            //     TIDE_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "' timeout("
        ...
        if (ac->err)
        {
            if (m_logEnable)
            {
                tide::replace(ctx->cmd, "\r\n", "\\r\\n");
                TIDE_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd
                                         << "' "
                                         << "(" << ac->err << ") " << ac->errstr;
            }
            if (ctx->fctx->fiber)
    ...
    //     TIDE_LOG_INFO(g_logger) << "DelayTimeCb";
    ...
    bool FoxRedis::init()
    {
        if (m_thread == tide::FoxThread::GetThis())
        {
            return pinit();
        }
        else
    ...
    bool FoxRedis::pinit()
    {
        // TIDE_LOG_INFO(g_logger) << "pinit m_status=" << m_status;
        ...
        if (!ctx)
        {
            TIDE_LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port
                                     << ") null";
            return false;
        }
        if (ctx->err)
        {
            TIDE_LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr;
            return false;
        }
        ctx->data = this;
        ...
        m_context.reset(ctx, tide::nop<redisAsyncContext>);
    ...
    ReplyPtr FoxRedis::cmd(const char *fmt, va_list ap)
    {
        ...
        if (len == -1)
        {
            TIDE_LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
            return nullptr;
        }
        ...
        //     free(buf);
        // }
        // ctx->scheduler = tide::Scheduler::GetThis();
        // ctx->fiber = tide::Fiber::GetThis();
        // ctx->thread = m_thread;

        FCtx fctx;
        fctx.cmd.append(buf, len);
        free(buf);
        fctx.scheduler = tide::Scheduler::GetThis();
        fctx.fiber = tide::Fiber::GetThis();
    ...
    ReplyPtr FoxRedis::cmd(const std::vector<std::string> &argv)
    {
        ...
        do
        {
            ...
            if (len == -1 || !buf)
            {
                TIDE_LOG_ERROR(g_logger) << "redis fmt error";
                return nullptr;
            }
            fctx.cmd.append(buf, len);
            free(buf);
    ...
    void FoxRedis::pcmd(FCtx *fctx)
    {
        if (m_status == UNCONNECTED)
        {
            TIDE_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port << ") unconnected "
                                    << fctx->cmd;
            init();
            if (fctx->fiber)
            {
    ...
    FoxRedis::Ctx::Ctx(FoxRedis *r)
        : ev(nullptr), timeout(false), rds(r)
          ...
          thread(nullptr)
    {
        tide::Atomic::addFetch(rds->m_ctxCount, 1);
    }

    FoxRedis::Ctx::~Ctx()
    {
        // cancelEvent();
        TIDE_ASSERT(thread == tide::FoxThread::GetThis());
        // TIDE_ASSERT(destory == 0);
        tide::Atomic::subFetch(rds->m_ctxCount, 1);
        //++destory;
        // cancelEvent();
        if (ev)
        {
    ...
    {
        // if(ev) {
        //     if(thread == tide::FoxThread::GetThis()) {
        //         evtimer_del(ev);
        //         event_free(ev);
        //     } else {
        //         auto e = ev;
    ...
    void FoxRedis::Ctx::EventCb(int fd, short event, void *d)
    {
        ...
        {
            tide::replace(ctx->cmd, "\r\n", "\\r\\n");
            TIDE_LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
    ...
    FoxRedisCluster::FoxRedisCluster(tide::FoxThread *thr, const std::map<std::string, std::string> &conf)
        : m_thread(thr), m_status(UNCONNECTED), m_event(nullptr)
    {
        m_ctxCount = 0;
    ...
    void FoxRedisCluster::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            TIDE_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb "
                                    << c->c.tcp.host << ":" << c->c.tcp.port
                                    << " success";
            if (!ar->m_passwd.empty())
            {
                ...
                if (rt)
                {
                    TIDE_LOG_ERROR(g_logger) << "FoxRedisCluster Auth fail: " << rt;
                }
            }
        }
        else
        {
            TIDE_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb "
                                     << c->c.tcp.host << ":" << c->c.tcp.port
                                     << " fail, error:" << c->errstr;
        }
    }
    ...
    {
        TIDE_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb "
                                << c->c.tcp.host << ":" << c->c.tcp.port
                                << " status:" << status;
    }
    ...
    bool FoxRedisCluster::pinit()
    {
        ...
        TIDE_LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
        ...
        if (!ctx)
        {
            TIDE_LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host
                                     << ") null";
            return false;
        }
        if (ctx->err)
        ...
        m_status = CONNECTED;
        // m_context.reset(ctx, redisAsyncFree);
        m_context.reset(ctx, tide::nop<redisClusterAsyncContext>);
    ...
    void FoxRedisCluster::pcmd(FCtx *fctx)
    {
        ...
        {
            TIDE_LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected "
                                    << fctx->cmd;
            init();
            if (fctx->fiber)
            {
    ...
    {
        // TIDE_LOG_INFO(g_logger) << "cancelEvent " << tide::FoxThread::GetThis()
        //            << " - " << thread
        //            << " - " << tide::IOManager::GetThis()
        //            << " - " << cancel_count;
        // if(thread != tide::FoxThread::GetThis()) {
        //     TIDE_LOG_INFO(g_logger) << "cancelEvent " << tide::FoxThread::GetThis()
        //                << " - " << thread
        ...
        //    //TIDE_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " " << thread->getId()
        //    //           << " this=" << tide::FoxThread::GetThis();
        //    //TIDE_ASSERT(thread == tide::FoxThread::GetThis());
        //}
        // TIDE_ASSERT(!tide::IOManager::GetThis());
        ////if(tide::Atomic::addFetch(cancel_count) > 1) {
        ////    return;
        ////}
        ////TIDE_ASSERT(!tide::Fiber::GetThis());
        ////tide::RWMutex::WriteLock lock(mutex);
        // if(++cancel_count > 1) {
        //     return;
        // }
        // if(ev) {
    ...
    IRedis::ptr RedisManager::get(const std::string &name)
    {
        ...
        if (r->getType() == IRedis::FOX_REDIS || r->getType() == IRedis::FOX_REDIS_CLUSTER)
        {
            ...
            return std::shared_ptr<IRedis>(r, tide::nop<IRedis>);
        }
        lock.unlock();
    ...
    void RedisManager::init()
    {
        ...
        for (auto &i : m_config)
        {
            ...
            auto pool = tide::TypeUtil::Atoi(get_value(i.second, "pool"));
            ...
            for (int n = 0; n < pool; ++n)
            {
                if (type == "redis")
                {
                    tide::Redis *rds(new tide::Redis(i.second));
                    rds->connect();
                    rds->setLastActiveTime(time(0));
                    tide::RWMutex::WriteLock lock(m_mutex);
                    m_datas[i.first].push_back(rds);
                    tide::Atomic::addFetch(done, 1);
                }
                else if (type == "redis_cluster")
                {
                    tide::RedisCluster *rds(new tide::RedisCluster(i.second));
                    rds->connect();
                    rds->setLastActiveTime(time(0));
                    tide::RWMutex::WriteLock lock(m_mutex);
                    m_datas[i.first].push_back(rds);
                ...
                else if (type == "fox_redis")
                {
                    auto conf = i.second;
                    auto name = i.first;
                    tide::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]()
                                                                {
                    tide::FoxRedis* rds(new tide::FoxRedis(tide::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                else if (type == "fox_redis_cluster")
                {
                    ...
                    tide::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done]()
                                                                {
                    tide::FoxRedisCluster* rds(new tide::FoxRedisCluster(tide::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
