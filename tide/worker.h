...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, tide::Scheduler* s = tide::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, tide::Scheduler* s = tide::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static tide::Logger::ptr s_logger = TIDE_LOG_NAME("system");
            TIDE_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef tide::Singleton<WorkerManager> WorkerMgr;
