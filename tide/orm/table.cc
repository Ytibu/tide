...
static tide::Logger::ptr g_logger = TIDE_LOG_NAME("orm");

std::string Table::getFilename() const {
    return tide::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if(!node.Attribute("name")) {
        TIDE_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if(!node.Attribute("namespace")) {
        TIDE_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
    ...
    if(!cols) {
        TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " columns is null";
        return false;
    }

    const tinyxml2::XMLElement* col = cols->FirstChildElement("column");
    if(!col) {
        TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " column is null";
        return false;
    }
    ...
    do {
        ...
        if(!col_ptr->init(*col)) {
            TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " init column error";
            return false;
        }
        if(col_names.insert(col_ptr->getName()).second == false) {
            TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
    ...
    if(!idxs) {
        TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " indexs is null";
        return false;
    }

    const tinyxml2::XMLElement* idx = idxs->FirstChildElement("index");
    if(!idx) {
        TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " index is null";
        return false;
    }
    ...
    do {
        ...
        if(!idx_ptr->init(*idx)) {
            TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " index init error";
            return false;
        }
        if(idx_names.insert(idx_ptr->getName()).second == false) {
            TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
        ...
        if(idx_ptr->isPK()) {
            if(has_pk) {
                TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " more than one pk"; 
                return false;
            }
            has_pk = true;
        }
        ...
        for(auto& x : cnames) {
            if(col_names.count(x) == 0) {
                TIDE_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + tide::replace(m_namespace, ".", "/");
    tide::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}
...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"tide/db/db.h", "tide/util.h"};
    for(auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = tide::split(m_namespace, '.');
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for(auto it = m_cols.begin();
        ...
        if((*it)->getDType() == Column::TYPE_UINT64
            ...
            ss << "tide::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
               << ";" << std::endl;
        } else {
    ...
    ss << "    return tide::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"tide/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static tide::Logger::ptr g_logger = TIDE_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v) \
    ...
    ofs << "        TIDE_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl << \
        "                 << \" errno=\"" \
