...
bool Index::init(const tinyxml2::XMLElement& node) {
    if(!node.Attribute("name")) {
        TIDE_LOG_ERROR(g_logger) << "index name not exists";
        return false;
    }
    m_name = node.Attribute("name");

    if(!node.Attribute("type")) {
        TIDE_LOG_ERROR(g_logger) << "index name=" << m_name << " type is null";
        return false;
    }
    ...
    if(m_dtype == TYPE_NULL) {
        TIDE_LOG_ERROR(g_logger) << "index name=" << m_name << " type=" << m_type
            << " invalid (pk, index, uniq)";
        return false;
    }

    if(!node.Attribute("cols")) {
        TIDE_LOG_ERROR(g_logger) << "index name=" << m_name << " cols is null";
    }
    std::string tmp = node.Attribute("cols");
    m_cols = tide::split(tmp, ',');
