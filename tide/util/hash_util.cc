...
#define ROTL(x, r) ((x << r) | (x >> (32 - r)))
    ...
    std::string replace(const std::string &str, char find, const std::string &replaceWith)
    {
        ...
        while (index != std::string::npos)
        {
            result = result.substr(0, index) + replaceWith + result.substr(index + 1);
            index = result.find(find, index + replaceWith.size());
        }
        return result;
    }
    ...
    std::string replace(const std::string &str, const std::string &find, const std::string &replaceWith)
    {
        ...
        while (index != std::string::npos)
        {
            result = result.substr(0, index) + replaceWith + result.substr(index + find.size());
            index = result.find(find, index + replaceWith.size());
        }
        return result;
    }
