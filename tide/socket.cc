...
namespace tide
{
    ...
    Socket::ptr Socket::accept()
    {
        ...
        int netsock = ::accept(m_sockfd, nullptr, nullptr);
        if (netsock == -1)
        {
            return nullptr;
        }

        if (sock->init(netsock))
        {
            return sock;
        }
    ...
    bool Socket::bind(const Address::ptr addr)
    {
        ...
        {
            TIDE_LOG_ERROR(g_logger) << "bind sock.family(" << m_family << ") addr.family(" << addr->getFamily() << ") not equal, addr=" << addr->toString();
            return false;
        }
    ...
    bool Socket::listen(int backlog)
    {
        if (!isValid())
        {
            TIDE_LOG_ERROR(g_logger) << "listen error, sockfd=" << m_sockfd << " is invalid";
            return false;
        }

        if (::listen(m_sockfd, backlog))
        {
            TIDE_LOG_ERROR(g_logger) << "listen error, errno=" << errno << " errstr=" << strerror(errno);
            return false;
        }
        return true;
    }
    ...
    bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms)
    {
        ...
        if (addr->getFamily() != m_family)
        {
            TIDE_LOG_ERROR(g_logger) << "connect sock.family(" << m_family << ") addr.family(" << addr->getFamily() << ") not equal, addr=" << addr->toString();
            return false;
        }
        ...
        if (timeout_ms == (uint64_t)-1)
        {
            ...
            {
                TIDE_LOG_ERROR(g_logger) << "connect error, errno=" << errno << " errstr=" << strerror(errno);
                close();
                return false;
            }
        }
    ...
    int Socket::recvFrom(void *buffer, size_t length, Address::ptr from, int flags)
    {
        if (isConnected())
        {
            socklen_t from_len = from->getAddrLen();
            return ::recvfrom(m_sockfd, buffer, length, flags, from->getAddr(), &from_len);
        }
        errno = ENOTCONN;
        return -1;
    }
    ...
    void Socket::newSock()
    {
        ...
        if (TIDE_LIKELY(m_sockfd != -1))
        {
            initSock();
        }
        else
        {
            TIDE_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", " << m_protocol << ") errno=" << errno << " errstr=" << strerror(errno);
        }
    }
    ...
    SSLSocket::ptr SSLSocket::CreateTCP(tide::Address::ptr address)
    {
